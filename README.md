...
`MT_SECRET` — это клиентский секрет из 32 шестнадцатеричных символов. Сгенерируйте новый:
...
Полученное значение запишите в `.env` (старое значение уже нельзя раздавать новым клиентам — при ротации обязательно сообщите всем пользователям новый секрет):

```bash
PUBLIC_HOSTNAME=proxy.example.com
MT_SECRET=<результат команды выше>
```
